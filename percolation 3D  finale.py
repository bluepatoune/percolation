...
# p = nombre de lignes dans chaque matrices         (y)
# q = nombre de coefficients dans chaque lignes     (z)
...
def maximum_alveoles(n, p, q, N, P):
...
def comptage_des_alveoles(espace, liste_vecteurs):
    """ renvoie le nombre d'alvéoles dans un espace """
...
def main (n, p, q, i, affichage = True):
    """ Renvoie le resultat de la percolation et le nombre d'alvéole pour un espace """
    espace = creation_espace(n, p, q, i)        # On crée un espace de départ de façon aléatoire
    liste_vecteurs = vecteurs_espace(DIM)   # Liste des vecteurs possibles déplacement dans l'espace
    return comptage_des_alveoles(espace, liste_vecteurs), modelisation(espace, liste_vecteurs, affichage) 
...
def percolation(espace, liste_vecteurs, eau_mouvante1, affichage):
    """Renvoie l'espace après propagationde l'eau (Affiche graphiquement la propagation)"""
    if affichage == True:
        fig = pyplot.figure()
...
def propagation(espace, eau_mouvante, liste_vecteurs):
    ...
    for (x, y, z) in eau_mouvante:
        pores_vides_locaux = vide(espace, x, y, z, liste_vecteurs)
